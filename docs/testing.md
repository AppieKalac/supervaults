...
5. Give the clean agent exactly the prompt. When a case has `clarification_policy`, answer only one agent question at a time before advancing the remaining `gate_script`; retain every question and exact reply.
   ...
   - For `constraint-packet-then-fallback`, answer the first clarification with one response formed by joining every `constraint_packet[].user_response` in listed order. This deliberately supplies the complete bounded contract regardless of the question's wording. Answer any later clarification, up to `max_turns`, with the exact `fallback_response`. Do not add another topic or case-specific reply. After `max_turns`, stop and score the case `inconclusive`.
