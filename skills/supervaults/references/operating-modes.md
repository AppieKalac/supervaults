...
- Apply that rule to every user-visible update and final. Never expose `Superpowers`, a `superpowers:<skill>` ID, or vendored-method branding in progress, questions, approvals, or handoffs; internal reads remain allowed.
...
2. **Internal method:** Read `../../vendor/superpowers/skills/brainstorming/SKILL.md` completely for new behavior or design uncertainty. Preserve its classification and approval gates. For a new project, ask one clarifying question at a time and include the proposed resolved vault destination in the design; do not initialize the vault, write a specification, create a plan, or implement before the applicable approval gate. If an approved architectural design proceeds to planning, read `../../vendor/superpowers/skills/writing-plans/SKILL.md` completely only after the planning prerequisites remain satisfied.
